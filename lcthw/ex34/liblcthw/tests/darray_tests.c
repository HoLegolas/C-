...
char *test_create()
{
	...
	mu_assert(array->end == 0, "end isn't at the right pos.");
	mu_assert(array->element_size == sizeof(int), "element's size is wrong.");
...
char *test_push_pop()
{
	...
	for (i=999; i>=0; i--) {
		...
		mu_assert(val != NULL, "Shouldd;t get a NULL");
