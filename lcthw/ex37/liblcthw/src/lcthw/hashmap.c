...
/**
 * Simple Bob Jenkin's hash algorithm taken from
 * the Wikipedia description
 */
static uint32_t default_hash(void *a)
{
	size_t len = blength((bstring) a); // get al length of a bstring
	char *key = bdata((bstring) a); // get data from bstring
	uint32_t hash = 0;
	uint32_t i = 0;
...
static inline int Hashmap_get_node(Hashmap *map, uint32_t hash, DArray *bucket, void *key)
{
	...
	for (i = 0; i < DArray_end(bucket); i++) {
		...
		if (node->hash == hash && map->compare(code->key, key) == 0) {
			return i;
		}
	}
	return -1;
...
void *Hashmap_get(Hashmap *map, void *key)
{
	...
	HashmapNode *node = DArray_get(bucket, i);
	check(node != NULL, "Failed to get not from bucket when it should exit.");
	return node->data;
error:
	return NULL;
}
...
void Hasmap_delete(Hashmap *map, void *key)
{
    uint32_t hash = 0;
    DArray *bucket = Hashmap_find_bucket(map, key, 0, &hash);
    if (!bucket)
